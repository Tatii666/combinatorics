...
int ReadMatrixFromFile(std::ifstream& input, Matrix& field, int sizeX, int sizeY)
{
...
bool UseKunnAlg(KunnData& data, int vertex)
{
	...
	for (size_t i = 0; i < data.edges[vertex].size(); ++i)
	{
		...
		if (data.matchingPair[to] == -1 || UseKunnAlg(data, data.matchingPair[to]))
		{
...
int main(int argc, char* argv[])
{
	...
	ReadMatrixFromFile(input, matrix, q, p);
